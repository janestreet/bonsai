...
val wrap_model :
  ?on_action_for_none:(unit, 'model option) Bonsai.on_action_mismatch ->
  ('input, 'model, 'result) Bonsai.t ->
  ('input, 'model option, 'result option) Bonsai.t
```
...
```ocaml
# open Bonsai.Infix
# let non_optional_model = 
  let%map.Bonsai input = Bonsai.input 
  and            model = Bonsai.model in
  sprintf "%f %d" input model
...
val assoc_model :
  ...
  ('input, 'model, 'result) Bonsai.t ->
  ('input, ('k, 'model, 'cmp) Core_kernel.Map.t,
