...
## <tt>Bonsai.Model.state\_machine</tt>

`Bonsai.Model.state_machine` is a function that can be used to define a component
solely in terms of its "apply action" function.

```ocaml
# #show Bonsai.Model.state_machine
val state_machine :
  Lexing.position ->
...
let component =
  let%map.Bonsai inject = Bonsai.Model.state_machine
    [%here]
