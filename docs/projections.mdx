...
However, by using `Bonsai.Model.ignore`, we can transform the
unit-modeled component to be whatever model type that we need.

```ocaml
# let combined =
  let%map.Bonsai counter = counter
  and some_constant = Bonsai.Model.ignore some_constant in
  Vdom.Node.div [] [ counter; some_constant ]
val combined : (unit, int, Vdom.Node.t) Bonsai.t = <abstr>
```

## <tt> Bonsai.Model.field </tt>
...
`Bonsai.Model.field` can be used to transform a Bonsai component from
a component whose model is the inner type, to a Bonsai component whose model is
the outer type.

```ocaml
# #show Bonsai.Model.field
val field :
...
# let age_counter = Bonsai.Model.field Person.Fields.age counter
...
```ocaml
let string_component = Bonsai.pure ~f:(Vdom.Node.text) |> Bonsai.Model.to_input
let int_component = counter
let person_component =
  let open Bonsai.Let_syntax in
  let%map age = int_component |> Bonsai.Model.field Person.Fields.age
  and name = string_component |> Bonsai.Model.field Person.Fields.name
...
that you can implement the `set` and `get` functions for those types.
...
val f :
  ...
  get:('m2 -> 'm1) ->
  set:('m2 -> 'm1 -> 'm2) -> ('input, 'm2, 'result) Bonsai.t
```
...
The `get` function converts from the target model type to the given model
type.  The `set` function takes an instance of the target model type and a
recently updated instance of the given model and asks the implementer of `get`
to produce a new target-model with the changes indicated by the provided model
incorporated into it.

If you didn't have `Bonsai.Model.field`, here's how you would implement
it using `Bonsai.Model.f`:

```ocaml
let project_field fieldslib_field t =
  Bonsai.Model.f t
    ~get:(Field.get fieldslib_field)
    ~set:(Field.fset fieldslib_field)
```
### Examples

Using `Bonsai.Model.f` it is possible to create a function that
...
With our friend `Model.to_input`, we can re-arrange the first two type
parameters of our display component and it'll work.

```ocaml
# let int_to_string_by_model = Bonsai.Model.to_input int_to_string_component
...
## <tt> Bonsai.Model.to\_input\_with\_other </tt>

This function is very similar to `Bonsai.Model.to_input`, with the main
difference being that the component has a secondary input that remains in input
position.

```ocaml
# #show Bonsai.Model.to_input_with_other
...
# let projected = Bonsai.Model.to_input_with_other string_repeat_component
...
```ocaml
# Bonsai.enum
- : ?on_action_mismatch:('key * 'model, 'model) Bonsai.on_action_mismatch ->
...
```ocaml
# Bonsai.if_
- : ?on_action_mismatch:(bool * 'model, 'model) Bonsai.on_action_mismatch ->
    ('input -> 'model -> bool) ->
