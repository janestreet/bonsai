...
let create_demo ~parameters =
  let first_panel =
    Value.return
      (Vdom.Node.div ~attr:(Vdom.Attr.class_ Styles.first_panel) [ Vdom.Node.text "1" ])
  in
  let second_panel =
    Value.return
      (Vdom.Node.div ~attr:(Vdom.Attr.class_ Styles.second_panel) [ Vdom.Node.text "2" ])
  in
  let%sub pane =
  ...
  let view =
    Node.div
      ~attr:(Attr.class_ Styles.resize_pane)
      [ Bonsai_web_ui_split_pane.to_vdom pane ]
  in
...
let app =
  ...
  Node.div
    ~attr:(Attr.class_ Styles.container)
    [ Node.div ~attr:(Attr.class_ Styles.demo) [ demo ]
    ; Node.div
        ~attr:(Attr.class_ Styles.parameters)
        [ parameters_form
        ; Node.button
