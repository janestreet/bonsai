...
(** Extends a Bonsai.t by providing a setter effect that can be used to override the
    returned value.  The computation will initially evaluate to the input [Bonsai.t].
...
(** Id_gen builds a component which generates unique identifiers by
    ...
    - [`Reset] fully resets the component, and will start generating ids starting at 0 again
    - [`Do_nothing] makes the component ignore the reset and keep its state as-is
    - [`Bump] moves the counter up one, fully disassociating it from the previous generation
      of ids.  This variant is only useful in [component'] when you'd want no previously generated
      ids to match the current id. *)
...
(** [mirror'] is like [mirror], but the incoming values have the type ['a option Bonsai.t]
    instead of just ['a Bonsai.t].  When a value is changed and its new value is None,
...
(** [chain_incr_effects input effects] allows you to sequentially schedule effects
    ...
    The outputs of each computation can be collected into a single [Bonsai.t],
