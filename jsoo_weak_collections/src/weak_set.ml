...
let rec t_of_js : 'a . (Ojs.t -> 'a) -> Ojs.t -> 'a t =
  fun (type __a) -> fun (__a_of_js : Ojs.t -> __a) -> fun (x2 : Ojs.t) -> x2
and t_to_js : 'a . ('a -> Ojs.t) -> 'a t -> Ojs.t =
  fun (type __a) -> fun (__a_to_js : __a -> Ojs.t) -> fun (x1 : Ojs.t) -> x1
let (create : unit -> 'a t) =
  fun () ->
    t_of_js Obj.magic
...
let (has : 'a t -> 'a -> bool) =
  fun (x8 : 'a t) ->
    fun (x7 : 'a) ->
      Ojs.bool_of_js
        (Ojs.call (t_to_js Obj.magic x8) "has" [|(Obj.magic x7)|])
