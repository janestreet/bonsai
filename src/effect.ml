...
module Callback = struct
  type ('request, 'response) t =
    | T :
        { request : 'request
        ; userdata : 'userdata
        ; on_response : 'response -> 'userdata -> Ui_event.t
        }
        -> ('request, 'response) t
...
type 'response t =
  | T :
      { request : 'request
      ; evaluator : ('request, 'response) Callback.t -> Ui_event.t
      }
      -> 'response t
...
module Private = struct
  ...
  let make ~request ~evaluator = T { request; evaluator }
end
...
let inject_ignoring_response t = inject t ~on_response:(Fn.const Ui_event.Ignore)
...
let never = T { request = (); evaluator = (fun _ -> Ui_event.Ignore) }

let sequence =
  let module E =
    Ui_event.Define (struct
      module Action = struct
        type t = (Ui_event.t, unit) Callback.t
      end

      let handle action =
        Ui_event.Expert.handle (Callback.request action);
...
let of_event (request : Ui_event.t) =
