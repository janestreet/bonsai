...
(** Converts a [Ui_event.t] to an [Effect.t].  This can be useful if you want
    to use the monadic interface that is available via [Effect.t] in order to
    sequence events. *)
val of_event : Ui_event.t -> unit t
...
val inject : 'a t -> on_response:('a -> Ui_event.t) -> Ui_event.t
...
val inject_ignoring_response : unit t -> Ui_event.t
...
val handle_error : ('ok, 'error) Result.t t -> f:('error -> Ui_event.t) -> 'ok t
...
module Private : sig
  module Callback : sig
    ...
    val make_with_userdata
      :  request:'a
      -> on_response:('b -> 'c -> Event.t)
      -> userdata:'c
      -> ('a, 'b) t

    val make : request:'a -> on_response:('b -> Event.t) -> ('a, 'b) t
    val request : ('a, 'b) t -> 'a
    val respond_to : ('a, 'b) t -> 'b -> Event.t
  end

  val make : request:'a -> evaluator:(('a, 'b) Callback.t -> Event.t) -> 'b t
end
