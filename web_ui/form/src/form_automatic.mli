...
(** [to_form2] turns the current [t] into a [Form2.t] by calling [Form.View.to_vdom] on
    the [view]. *)
val to_form2 : 'a t -> ('a, Vdom.Node.t) Form_manual.t

(** [to_form2'] is identical to [to_form2], but it handles the [let%arr] for you. *)
val to_form2' : 'a t Value.t -> ('a, Vdom.Node.t) Form_manual.t Computation.t

(** [of_form2] creates a [t] from a [Form2.t]. Unlike [to_form2], this function needs to
    be stateful because creating a [t] requires a [unique_key] which is generated with
    [Bonsai.path_id]. *)
val of_form2 : ('a, Vdom.Node.t) Form_manual.t Value.t -> 'a t Computation.t
