...
let%expect_test "sorting default renderer" =
  let multisort_columns_when = Bonsai.Var.create `Shift_click in
  let test =
    Test.create
      (Test.Component.default
         ~multisort_columns_when:(Bonsai.Var.value multisort_columns_when)
         ())
  in
  Handle.recompute_view_until_stable test.handle;
  Handle.show test.handle;
  ...
  Bonsai.Var.set multisort_columns_when `Ctrl_click;
  Handle.recompute_view_until_stable test.handle;
  Handle.click_on
    ~ctrl_key_down:true
    test.handle
  ...
  Bonsai.Var.set multisort_columns_when `Shift_or_ctrl_click;
  Handle.recompute_view_until_stable test.handle;
  Handle.click_on
    ~ctrl_key_down:true
    test.handle
  ...
  Bonsai.Var.set multisort_columns_when `Shift_click;
  Handle.recompute_view_until_stable test.handle;
  Handle.click_on
    ~ctrl_key_down:true
    test.handle
